...
)
{
  ...
  if ( $grid_base == undef or ($grid_base =~ String) == false) {fail('You must specify a grid_base') }
  if ( $grid_home == undef or ($grid_home =~ String) == false) {fail('You must specify a grid_home') }
